...
export default function Footer() {
  return (
    <footer className="py-16 px-6" style={{ backgroundColor: 'var(--color-ink)' }}>
      <div className="max-w-6xl mx-auto grid sm:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
        {/* Brand */}
        <div className="lg:col-span-1">
          <div className="flex items-center gap-2.5 mb-4">
            <div className="w-9 h-9 rounded-[var(--radius-md)] flex items-center justify-center" style={{ backgroundColor: 'var(--color-spark)' }}>
              <span className="text-sm font-bold" style={{ color: 'var(--color-ink)' }}>O</span>
            </div>
            <span className="text-xl font-bold tracking-tight text-white">ORIN</span>
          </div>
          <p className="text-sm text-white/60 leading-relaxed">
            Turn scattered work into verified career proof. Built for students, by students.
          </p>
        </div>
      ...
      <div className="max-w-6xl mx-auto border-t border-white/10 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4">
        <p className="text-sm text-white/40">&copy; {new Date().getFullYear()} ORIN. All rights reserved.</p>
